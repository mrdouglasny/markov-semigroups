...
| `ouSemigroupFin_entropy_sq_decay_bound` | [`Instances/WorkInProgress/EuclideanFin.lean`](../MarkovSemigroups/Instances/WorkInProgress/EuclideanFin.lean) (now a `theorem`) | Bakry-Gentil-Ledoux Theorem 5.5.2 / BGL §5.5 (1D analogue was historically axiomatized at `Euclidean.lean:943` and proved in commit `1b3f797` via A1+A2 decomposition) | **PARTIAL — in progress (1 `sorry`, no longer an `axiom`)** | GR (gemini-3.1-pro-preview 2026-05-13 statement vet **Standard**; deep-think 2026-05-19 confirmed the **telescoping route**; deep-think + 3.1-pro 2026-05-19 **flagged the C∞-iterate sub-lemma as mathematically FALSE** — see below) | **Workstream N1.c, 2026-05-19:** `axiom` → `theorem` with the telescoping route partially formalized. **Fully proved axiom-free supporting lemmas:** (a) `Gaussian1D.boltzmannEntropy_ouSemigroup_decay_le`; (b) `entropy_sub_eq_boltzmann_sub`; (c) `boltzmannEntropyFin_ouCoord_step_le` (per-coordinate step, **requires `ContDiff ℝ ∞`**); (d) `sum_fisherInfoFinCoord_sq_add_const_le` (T3 energy); ε→0 DCT tail (`boltzmannSubFin_le_of_perEps` + `ouSemigroupFin_sq_add_const`). **T1 factorization now FULLY PROVED & axiom-free** (this session): `Gaussian1D.integral_ouSemigroup_eq`, `integral_ouCoord_eq`, `setShift_insert_update`, `integral_update_swap`, `ouCoord_ouCoordSet` (composition step `ouCoord j ∘ ouCoordSet S = ouCoordSet (insert j S)`, `j∉S`), `measurable_setShift`/`measurable_ouCoordSet`, `ouCoordSet_bounds`, `integral_ouCoordSet_eq` (measure preservation). **DISCOVERED BLOCKER:** the prior plan fed telescope iterates `ouCoordSet S_k t g_ε` into the C∞-requiring step lemma; `IsCoreFin (ouCoord j t g_ε)` is **mathematically false** — single-coordinate Mehler smooths only the integrated coordinate, passthrough coords inherit `g`'s regularity with no gain, so under `IsCoreFin` (≤2nd-order bounds) `ouCoord j t g` is only **`C²` jointly, not `C∞`** (3rd passthrough derivative diverges for `t>½ln2`; explicit counterexample). Corrected route documented in-file (S1–S5): introduce a `C²` core predicate, prove `ouCoordSet` preserves it, weaken the step lemma to `ContDiff ℝ 2`, one-shot orthogonal Fisher monotonicity, telescope. `#print axioms = [propext, sorryAx, Classical.choice, Quot.sound]`; **all new T1 helper lemmas verified `[propext, Classical.choice, Quot.sound]` (no new axioms)**. | The `semigroup_entropy_sq_decay_bound` field of the BE-instance wrap. Downstream: same as for the other two GaussianFin axioms — eventual gaussian-hilbert `ouSemigroupAct_eLpNorm_hypercontractive` discharge. |
