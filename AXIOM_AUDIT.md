...
| `ouSemigroupFin_entropy_sq_decay_bound` | [`Instances/WorkInProgress/EuclideanFin.lean`](../MarkovSemigroups/Instances/WorkInProgress/EuclideanFin.lean) (now a `theorem`) | Bakry-Gentil-Ledoux Theorem 5.5.2 / BGL §5.5 (1D analogue was historically axiomatized at `Euclidean.lean:943` and proved in commit `1b3f797` via A1+A2 decomposition) | **PARTIAL — in progress (1 `sorry`, no longer an axiom)** | GR (gemini-3.1-pro-preview 2026-05-13 statement vet **Standard**; deep-think 2026-05-19 confirmed the **telescoping route**) | **Workstream N1.c, 2026-05-19:** `axiom` converted to `theorem` with the telescoping route partially formalized. **Fully proved supporting lemmas (all axiom-free — `[propext, Classical.choice, Quot.sound]`):** (a) `Gaussian1D.boltzmannEntropy_ouSemigroup_decay_le` — general 1D Boltzmann-entropy decay `H(g) − H(P_t g) ≤ (1−e^{−2t})/2·I(g)` (FTC assembly of A1+A2), in `EuclideanEntropyDecay.lean`; (b) `entropy_sub_eq_boltzmann_sub` — macroscopic `(∫·)log(∫·)` cancellation reducing the centered-entropy difference to the Boltzmann difference (via `ouSemigroupFin_integral_eq_of_bound`); (c) `boltzmannEntropyFin_ouCoord_step_le` — the **per-coordinate step bound** `BE(h) − BE(ouCoord i t h) ≤ (1−e^{−2t})/2·I_i(h)` (1D lemma slicewise + swap-Fubini `integral_γFin_succAbove_swap` + parametric measurability + integrability + `integral_mono`); (d) `sum_fisherInfoFinCoord_sq_add_const_le` — **energy bookkeeping** `Σ_i I_i(f²+ε) ≤ 4·ouEnergyFin f f`. Plus `ouCoord`, `ouCoord_insertNth_eq`, `update_insertNth_same`, `slice_eq_coordSection`, `boltzmannEntropyFin`, `fisherInfoFinCoord`, `partialDeriv_sq_add_const`. **2026-05-19 follow-up (this branch):** discharged the **ε→0 DCT tail** as the standalone axiom-free lemma `boltzmannSubFin_le_of_perEps` (n-dim analogue of the proved 1D ε-tail) and the helper `ouSemigroupFin_sq_add_const` (`P_t(f²+ε)=P_t(f²)+ε`); added the **T1 factorization scaffolding** `setShift` / `ouCoordSet` with `ouCoordSet_empty` (`= id`) and `ouCoordSet_univ` (`= ouSemigroupFin t`), all axiom-free. The main theorem now discharges Step 1 (entropy→Boltzmann) **and** Step 2 (ε-reduction, via `refine boltzmannSubFin_le_of_perEps …`) inside the proof body; the single remaining `sorry` is now the **per-ε telescoping core only**: T1 composition step `ouCoord j t ∘ ouCoordSet S t = ouCoordSet (insert j S) t` (`j∉S`; elementary `setShift` `funext` + same-dim `(s,y)↦update y j s` measure-preservation from `measurePreserving_piFinSuccAbove`/`integral_γFin_succAbove`) + T2 orthogonal Fisher monotonicity `I_k(ouCoord j t h) ≤ I_k h` for `j≠k` (commutation + kernel Cauchy–Schwarz mirroring proved 1D `ouSemigroup_fisher_info_decay`; needs `IsCoreFin (ouCoord j t g_ε)` regularity — port of `hasFDerivAt_ouSemigroupFin`/`contDiffOne_ouSemigroupFin` to the diagonal shift, the hardest sub-lemma) + telescope sum. T3 (`sum_fisherInfoFinCoord_sq_add_const_le`) already proved. `#print axioms = [propext, sorryAx, Classical.choice, Quot.sound]`; all new helper lemmas verified axiom-free `[propext, Classical.choice, Quot.sound]` (no new axioms). | The `semigroup_entropy_sq_decay_bound` field of the BE-instance wrap. Downstream: same as for the other two GaussianFin axioms — eventual gaussian-hilbert `ouSemigroupAct_eLpNorm_hypercontractive` discharge. |
