...
| `EuclideanFin.lean` (+ `EuclideanFinBE.lean`) | Multivariate Gaussian: standard product Gaussian `γFin n = Measure.pi γ` on `Fin n → ℝ`, finite-dim Bakry–Émery data for the Stage-N1 / Phase-2.5 construction. | BGL-2014 Ch. 2 (tensorized OU); Ledoux-2000 §1 (LSI/Poincaré tensorize, (1.23)–(1.24)). | **0 sorry, 0 axiom in-file.** The finite-dimensional BE layer is theorem-only. The multivariate de Bruijn derivative `ouSemigroupFin_l2_sq_hasDerivWithinAt` is a **theorem** in `EuclideanFinBE.lean`; after the 2026-05-22 discharges of `gaussianFin_integrationByParts` in `EuclideanGeneratorLp.lean`, `gaussianOU_heatEquation_within_zero` in `EuclideanGeneratorLimit.lean`, and `gaussianFin_diffQuot_tendsto_Lp` in the same file, the live GaussianFin Gross chain is now fully axiom-free. |
...
| `TwoPoint.lean` | Two-point space `{0,1}`, uniform `μ=½(δ₀+δ₁)`, jump chain `P_t f(i)=½(1+e^{-2αt})f(i)+½(1−e^{-2αt})f(1−i)`, `ρ=2α`. Simplest would-be `BakryEmerySpace` instance — and a deliberate counterexample probe. | Elementary discrete two-point Markov chain (folklore / Ledoux-2000 §1 discrete-Γ remarks; Diaconis-style two-state continuous-time chain). | **0 sorry, 0 axiom (2026-05-22).** Previously a sorry-backed (hence unsound) `BakryEmerySpace TwoPoint` instance whose `Γ_leibniz` / entropy-decay fields are mathematically *false* for a jump process. Now the impossible instance is removed and the failure is **proved**: `gamma_leibniz_fails` exhibits the explicit counterexample (Leibniz LHS `α/2` vs RHS `α` at `.zero`). The instance demonstrates *which* `BakryEmerySpace` fields fail off the diffusion setting — by proving they fail, not by stubbing them. |
