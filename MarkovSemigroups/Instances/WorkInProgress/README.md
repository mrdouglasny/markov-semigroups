...
| `EuclideanFin.lean` (+ `EuclideanFinBE.lean`) | Multivariate Gaussian: standard product Gaussian `γFin n = Measure.pi γ` on `Fin n → ℝ`, finite-dim Bakry–Émery data for the Stage-N1 / Phase-2.5 construction. | BGL-2014 Ch. 2 (tensorized OU); Ledoux-2000 §1 (LSI/Poincaré tensorize, (1.23)–(1.24)). | **0 sorry, 2 axioms.** Remaining real `axiom` decls (verified): `ouSemigroupFin_preserves_IsCore`, `ouSemigroupFin_entropy_sq_decay_bound`. The multivariate de Bruijn derivative `ouSemigroupFin_l2_sq_hasDerivWithinAt` is now a **theorem** in `EuclideanFinBE.lean`, discharged via the already-counted G2 axioms `gaussianFin_diffQuot_tendsto_Lp` and `gaussianFin_integrationByParts`. |
