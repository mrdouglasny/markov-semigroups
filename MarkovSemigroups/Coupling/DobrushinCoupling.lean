...
/-- **Dobrushin iterated coupling existence (compact spin space).**

Same statement as `dobrushin_coupling_axiom`, but for compact (not necessarily
finite) spin spaces S. Replaces `[Fintype S] [MeasurableSingletonClass S]`
with `[TopologicalSpace S] [CompactSpace S] [T2Space S]
[SecondCountableTopology S] [BorelSpace S]`.

**Proof sketch (not yet formalized):**
1. `Omega = SpinConfig I S x SpinConfig I S` is compact (Tychonoff + product).
2. `ProbabilityMeasure Omega` is compact (Prokhorov).
3. The coupling set `{nu | nu.map fst = mu_1, nu.map snd = mu_2}` is closed
   (preimage of a closed singleton under the continuous map
   `ProbabilityMeasure.map`, using `ProbabilityMeasure.continuous_map`).
4. Total disagreement `D(nu) = sum_z nu({p | p.1 z != p.2 z}).toReal` is
   lower semicontinuous (each `nu -> nu(U).toReal` is lsc for open U by
   Portmanteau; finite sums of lsc functions are lsc).
5. By `LowerSemicontinuousOn.exists_isMinOn`, D attains its minimum on the
   compact coupling set.
6. The rest of the argument (minimality implies per-site contraction) is
   identical to the finite case.

**Formalization cost.** Replacing this axiom with a proof requires:
- Formalizing lower semicontinuity of `nu -> nu(U).toReal` for open U
  on `ProbabilityMeasure` (Portmanteau direction, ~100 lines)
- Adapting the measurability arguments from `of_discrete` to Borel (~150 lines)
- Total: approximately 250-400 lines of infrastructure.

References:
- Dobrushin (1968), Lemma 2
- Georgii (1988), Proposition 8.7
- Prokhorov (1956), compactness of probability measures -/
axiom dobrushin_coupling_axiom_compact
