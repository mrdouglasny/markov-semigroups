...
| `stroock_varopoulos` | BGL Prop 1.7.1 / §1.7 (Stroock 1992, Varopoulos 1985) | First intermediate-step lemma toward a Gross discharge. `(4(p-1)/p²) · E(f^{p/2}, f^{p/2}) ≤ E(f, f^{p-1})` for nonneg `f` in core with `f^{p/2}, f^{p-1}` in core and `p ≥ 2`. Vetted Likely-correct by gemini-3.1-pro-preview 2026-05-13; original `f ≥ ε > 0` revised to `f ≥ 0` per Gemini (powers C¹ at 0 for p ≥ 2). No internal consumers yet. |
